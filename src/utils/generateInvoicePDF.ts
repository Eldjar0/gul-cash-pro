...
export const generateInvoicePDF = async (invoice: InvoiceData): Promise<jsPDF> => {
  ...
  const labelRightX = tableRightEdge - (3 * colWidth) - 2; // Labels alignés à droite avant les colonnes
  
  // En-têtes des colonnes (centrés dans chaque colonne)
  doc.setFontSize(6);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(100, 100, 100);
  doc.text('HTVA', htvaCenter, yPos, { align: 'center' });
  doc.text('TVA', tvaCenter, yPos, { align: 'center' });
  doc.text('TTC', ttcCenter, yPos, { align: 'center' });
  yPos += 4;
